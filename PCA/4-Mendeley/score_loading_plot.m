...
% OPTIONAL: Pass in the names of the data labels 
%       If the user does not pass in anything for Dataset, the code will
%       just label each variable in the loadings as "Var #" where "#" is
%       the ordered number of the column of X for that loading
...
for i = 1:length(p1)
    ...
    plot(p1(i)*xlim(1),p2(i)*ylim(1),'rs','MarkerSize',10,'MarkerFaceColor','r');
    
    switch nargin
        case 5
            text(p1(i)*xlim(1)+0.0,p2(i)*ylim(1)+0.5, Dataset(i),'FontSize',12,'Color','red','HorizontalAlignment','center');
        otherwise
            text(p1(i)*xlim(1)+0.0,p2(i)*ylim(1)+0.2, ['Var ', num2str(i)],'FontSize',12,'Color','red');
    end
end
