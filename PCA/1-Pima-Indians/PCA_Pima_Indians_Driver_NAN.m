...
%% Import dataset
data = readmatrix("pima-indians-diabetes-NAN-included.csv");
X = data(:,[2 7 8]);
Y = data(:,end);
...
X_CS = zeros(N,K);
for k = 1:K
    X_ctr(:,k) = X(:,k) - mean(X(:,k),"omitnan");
    X_CS(:,k) = X_ctr(:,k)/std(X_ctr(:,k), "omitnan");
end %for
